...
namespace components::edge_detection {
    ...
    void SobelEdgeDetectionGPU::applySobelFilter() {
        const cl::Buffer img_buffer{ cl_context, CL_MEM_READ_ONLY, img_size * sizeof(float) };
        queue.enqueueWriteBuffer(img_buffer, CL_FALSE, 0, img_size * sizeof(float), inputImage.data());

        const cl::Buffer out_buffer{ cl_context, CL_MEM_READ_WRITE, img_size * sizeof(float) };
