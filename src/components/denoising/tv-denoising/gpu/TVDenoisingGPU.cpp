...
namespace components::denoising {
    ...
    float TVDenoisingGPU::tvNormAndGrad() {
        ...
        const float tv_norm{ sum<float>(cl_context, queue, utils_program, tvNormMtxBuffer, img_size) };

        const std::string kernel_name{ "grad_from_dx_dy" };
    ...
    float TVDenoisingGPU::l2NormAndGrad() {
        ...
        const float l2_norm{ sum<float>(cl_context, queue, utils_program, l2NormMtxBuffer, img_size) };
